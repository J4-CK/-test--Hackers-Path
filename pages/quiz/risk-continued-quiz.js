...
export default function QuizPage() {
  ...
  const saveScore = async (finalScore) => {
    ...
    try {
      const response = await fetch('/api/quiz/save-score', {
        ...
        body: JSON.stringify({
          quizName: 'Risk Management Continued Quiz',
          score: finalScore,
  ...
  const handleLogin = () => {
    router.push('/login?returnUrl=' + encodeURIComponent('/quiz/risk-continued-quiz'));
  };
  ...
  if (loading) {
    return (
      <div>
        <Head>
          <title>Risk Management Continued Quiz</title>
